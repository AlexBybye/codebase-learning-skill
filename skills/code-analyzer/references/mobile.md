...
逐跳记录 `source/evidence`。层不存在时直接连接实际调用方和被调方；实现位于仓库外时标为 `unavailable`，不能套用固定分层。若请求进入同仓库 backend，引用 backend 单元登记的 handler。
...
- 进入/离开导航、触发条件和参数；
- 用户操作与生命周期加载链；
- 客户端请求、本地存储、缓存和错误/空态处理；
- 仅能由源码证明的颜色、尺寸、图标或设计 token。

不从命名猜测 UI，不补造夜间样式，不把项目专有网络客户端或设计系统当默认值。
